...
def asv_sounder_animation_core(fig, sp_e, sp_n, elev, c_min, c_max, bad_lat, bad_lon, sonar_time, bs, inst_depth,
                               smooth_depth, gnss_loc, fspec, fspec_bin):
    ## in function #############
...
def asv_sounder_animation(figure_out_base, d, wave, image_fname, plot_every_x_data_points=20):
    ...
    for ttt in tqdm.tqdm(np.arange(dont_plot_before_x_data_points, len(d['time'])).astype(int)[::plot_every_x_data_points],
                         desc='ASV animation'):
        ...
        inst_depth, smooth_depth = d['sonar_instant_depth'][ttt-sounder_tail:ttt], d['sonar_smooth_depth'][ttt-sounder_tail:ttt]
        gnss_loc = d['gnss_elevation_navd_m'][:ttt]
        if np.size(sonar_time) < dont_plot_before_x_data_points: continue
        ...
        fig = asv_sounder_animation_core(fig, sp_e, sp_n, elev, c_min, c_max, bad_lat, bad_lon, sonar_time, bs, inst_depth,
                                         smooth_depth, gnss_loc, fspec, fspec_bin)
        # fig.tight_layout()
...
start = nc.num2date(d['time'][0], 'seconds since 1970-01-01', only_use_python_datetimes=True, only_use_cftime_datetimes=False)
start = start.replace(minute=0, second=0, hour=0, microsecond=0)
end = nc.num2date(d['time'][-1], 'seconds since 1970-01-01', only_use_python_datetimes=True, only_use_cftime_datetimes=False)
